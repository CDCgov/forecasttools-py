...
def weekly_rt_nowcast(y=None):
    ...
    rt_walk = numpyro.sample("random_walk", dist.Normal(rt, 0.2).expand([10]))
...
def daily_hospitalizations(observations=None):
    ...
    lambda_ = numpyro.sample("lambda", dist.Exponential(1.5))
...
def run_mcmc(model, obs_data, num_samples=500, num_warmup=200):
    """Gets MCMC object for a given model and observational data"""
    rng_key = jr.key(0)
    kernel = numpyro.infer.NUTS(model)
    mcmc = numpyro.infer.MCMC(
        kernel, num_samples=num_samples, num_warmup=num_warmup
    )
    mcmc.run(
        rng_key,
        y=obs_data if "y" in model.__code__.co_varnames else None,
        obs=obs_data if "obs" in model.__code__.co_varnames else None,
        observations=(
            obs_data if "observations" in model.__code__.co_varnames else None
        ),
    )
    return mcmc
...
mcmc_weekly = run_mcmc(weekly_rt_nowcast, weekly_data)
mcmc_biweekly = run_mcmc(biweekly_hospitalizations, biweekly_data)
mcmc_daily = run_mcmc(daily_hospitalizations, daily_data)
