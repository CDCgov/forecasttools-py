...
check_and_install_r_packages(c("dplyr", "ggplot2", "readr", "arrow", "scoringutils"))
...
_This section explores using `scoringutils` for scoring the quantile forecasts made in a FluSight Hub submission._

While `forecasttools` has an example FluSight submission (in hubverse format) available, `scoringutils` requires observed data
...
Scoring utils requires observed data to be loaded.
...
# read parquet file
df <- read_parquet("forecasts_for_scoringutils.parquet")
