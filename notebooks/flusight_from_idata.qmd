...
def loc_abbr_to_flusight_code(
    ...
    mapping_dict = dict(zip(loc_table["short_name"], loc_table["location_code"]))
    # replace column
    df = df.with_columns([
        pl.col(location_col).map_elements(lambda x: mapping_dict.get(x, None)).alias(location_col)
    ])
    return df

forecast_df = loc_abbr_to_flusight_code(df=forecast_df, location_col="location")
```
